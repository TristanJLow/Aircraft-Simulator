...
public class SimulationRunner {
	...
	public void runSimulation() throws AircraftException, PassengerException, SimulationException, IOException {
		...
		for (int time=0; time<=Constants.DURATION; time++) {
			...
			GUISimulator.createQ_RData(time,sim.numInQueue(),sim.numRefused());
			
			if(time > 20){
			    Bookings data = sim.getFlights(time).getCurrentCounts();
			    GUISimulator.createFlightData(time, data.getNumFirst(),data.getNumBusiness(),data.getNumPremium(),data.getNumEconomy(),data.getAvailable(),data.getTotal());
			}
		}
