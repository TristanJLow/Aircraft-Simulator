...
public abstract class Passenger {
	...
	public Passenger(int bookingTime, int departureTime) throws PassengerException  {
		// Checks if variables input are valid.
		if (bookingTime < 0 || departureTime <= 0 || departureTime < bookingTime) {
			throw new PassengerException("Constructor Exception");
		} else {
	...
	public void cancelSeat(int cancellationTime) throws PassengerException {
		// Statements to check if the state of the passenger is valid to be cancelled.
		if (this.newState) {
			...
			throw new PassengerException("Departure time before cancellation");
		} else {
			// Cancels the booking
	...
	public void confirmSeat(int confirmationTime, int departureTime) throws PassengerException {
		// Statements to check if the state of the passenger is valid to be confirmed.
		if (this.confirmed) {
			...
			throw new PassengerException("Departure time is before confirmation");
		} else {
			// Confirms the seat for the passenger.
		    if(this.newState || this.inQueue) {
		        if(this.inQueue){
	...
	public void flyPassenger(int departureTime) throws PassengerException {
		// Checks if the current state of the passenger is valid to be changed to flown.
		if(this.newState || this.inQueue || this.refused || this.flown || departureTime <= 0){
		    throw new PassengerException("newState/inQueue/refused/flown == true or departureTime <= 0");
		} else {
			// Changes the passengers state to flown.
		    if(this.confirmed){
		        this.confirmed = false;
		        this.flown = true;
	...
	public void queuePassenger(int queueTime, int departureTime) throws PassengerException {
		// Checks if the current state of the passenger is valid to be changed to in queue.
		if (this.inQueue) {
			throw new PassengerException("Is in queue");
		}
			...
			throw new PassengerException("Departure time before queue time");
		} else {
	...
	public void refusePassenger(int refusalTime) throws PassengerException {
		// Checks if the current state of the passenger is valid to be refused.
		if (this.confirmed) {
			throw new PassengerException("Is confirmed");
		}
		else if (this.refused) {
			...
			throw new PassengerException("refusal time is less than the booking time");
		} else {
			// Refuses the passenger.
		    if(this.newState || this.inQueue){
		        if(this.inQueue){
	...
	protected void copyPassengerState(Passenger p) {
		// Makes a duplicate of the passenger that is passed into the method.
